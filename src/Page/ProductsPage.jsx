...
const ProductsPage = () => {
    ...
    const fetchPaintings = async () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          // Caching: Local storage mein save kar rahe hain taake foran load ho
          localStorage.setItem("cached_art", JSON.stringify(res.data.data));
        }
      } catch (err) {
        console.error("❌ API Error:", err.message);
        setError("Backend se connection nahi ho paa raha.");
        
        // Agar net slow ho ya server band ho to purana cached data dikhao
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex items-center justify-center flex-col gap-4">
      ...
      <p className="text-white font-mono tracking-widest animate-pulse">FETCHING FROM CLOUD...</p>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#050505] text-white pt-32 pb-20 px-5">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-2 mb-10">
          <div className={`w-2 h-2 rounded-full ${error ? 'bg-red-500' : 'bg-green-500'} animate-ping`}></div>
        ...
        <div className="mb-20 text-center">
          ...
          <p className="text-gray-500 mt-4 font-mono tracking-widest text-xs">Total Items in Collection: {paintings.length}</p>
        </div>

        {paintings.length === 0 ? (
          <div className="text-center py-20 bg-white/5 rounded-[40px] border border-dashed border-white/10">
            <FaDatabase className="mx-auto text-gray-700 mb-4 size-10" />
            <p className="text-gray-500 font-mono italic">"The gallery is currently empty. Add paintings via Postman/Backend to see them here."</p>
          </div>
        ) : (
            ...
            {paintings.map((art) => (
              ...
              >
                <div className="h-[400px] overflow-hidden relative">
                  <img src={art.imageUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale group-hover:grayscale-0" alt={art.title} />
                ...
                <div className="p-6">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <p className="text-[#0b6472] font-black text-xl">${art.price}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-5">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tighter">{art.artist}</span>
                    </div>
                    <button className="text-[11px] font-bold text-white bg-[#0b6472] px-5 py-2 rounded-full hover:scale-105 transition-transform uppercase">View Details</button>
                  </div>
                </div>
              </motion.div>
            ))}
